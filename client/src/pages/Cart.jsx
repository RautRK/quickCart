...
const Cart = () => {
    ...
    // Safeguard: Get cart data with valid values
    const getCart = () => {
        let tempArray = [];
    ...
    // Safeguard: Place order with valid total amount check
    const placeOrder = async () => {
        try {
            ...
            const totalAmount = getCartAmount() + getCartAmount() * 2 / 100; // Include tax

            // Check for NaN and ensure valid amount
            if (isNaN(totalAmount) || totalAmount <= 0) {
                return toast.error("Invalid total amount. Please check your cart.");
            }
            ...
            if (paymentOption === "COD") {
                const { data } = await axios.post("/api/order/cod", {
                    ...
                    amount: totalAmount, // Send the valid amount
                });
            ...
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product) => (
                    <div key={product._id} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <p className="text-center">{currency}{product.offeredPrice * product.quantity}</p>
